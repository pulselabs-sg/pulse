        ...
        <div className="flex-1 overflow-y-auto p-3 md:p-10 w-full custom-scrollbar">
            <div className="w-full max-w-5xl mx-auto space-y-8">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/5 pb-6">
                    <div>
                        <h2 className="text-2xl font-bold tracking-tight text-white mb-1 flex items-center gap-3 uppercase text-glow-cyan">
                            <History className="w-6 h-6 text-cyan-400" /> History
                        </h2>
                        <p className="text-zinc-400 font-mono text-[10px] tracking-[0.2em]">Archived neural synthesis cycles</p>
                    </div>
                    {history.length > 0 && (
                        <div className="text-[10px] font-mono text-zinc-400 uppercase glass-mid px-3 py-1 rounded-full border border-white/5 shadow-[0_0_15px_rgba(34,211,238,0.1)]">
                            Total Records: <span className="text-white text-glow-cyan">{history.length}</span>
                        </div>
                    )}
                </div>
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center p-20 gap-4">
                        ...
                        <p className="text-[10px] font-mono text-zinc-600 tracking-widest text-glow-cyan">Accessing secure archives...</p>
                    </div>
                ) : history.length === 0 ? (
                        ...
                        <AnimatePresence mode="popLayout">
                            {history.map((item, index) => (
                                <motion.div
                                    ...
                                    className="glass-dark border border-white/10 rounded-2xl p-4 md:p-5 flex flex-col md:flex-row gap-4 md:gap-6 items-start md:items-center hover:border-cyan-500/30 hover:bg-white/[0.02] transition-all group cursor-default overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.3)] relative"
                                >
                                    ...
                                    <div className="shrink-0 flex items-center gap-3 w-auto md:w-32">
                                        <div className={cn(
                                            ...
                                            item.type?.toLowerCase() === 'stt' ? "bg-purple-500/10 border-purple-500/20 text-purple-400" : "bg-cyan-500/10 border-cyan-500/20 text-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.2)]"
                                        )}>
                                            {item.type?.toLowerCase() === 'stt' ? <FileText className="w-4 h-4" /> : <Music className="w-4 h-4" />}
                                        </div>
                                        <span className={cn("text-[10px] font-bold uppercase tracking-widest relative z-10", item.type?.toLowerCase() !== 'stt' && "text-glow-cyan")}>
                                            {item.type}
                                        </span>
                                    </div>
