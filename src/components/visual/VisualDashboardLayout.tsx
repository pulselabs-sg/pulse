...
import {
  Settings2, Mail, ShieldCheck, LogOut, CreditCard, Loader2,
  X, Sparkles, Check, Crown, Menu, FileText, Scale, Lock,
...
import SidebarVisual from '@/components/visual/SidebarVisual';
...
export default function VisualDashboardLayout({ projectId }: VisualDashboardLayoutProps) {
  const { data: session, status, update } = useSession();
  const router = useRouter();
  const searchParams = useSearchParams();
  const successParam = searchParams.get('success');

  const tabParam = searchParams.get('tab') as VisualTab | null;
  const validTabs: VisualTab[] = ['imagine', 'history', 'profile'];
  const initialTab: VisualTab = tabParam && validTabs.includes(tabParam) ? tabParam : 'imagine';
  
  const [activeTab, setActiveTab] = useState<VisualTab>(initialTab);
  ...
  // Terms & Checkout Agreement States
  ...
  useEffect(() => {
    if (tabParam && validTabs.includes(tabParam as VisualTab) && tabParam !== activeTab) {
      setActiveTab(tabParam as VisualTab);
    }
  }, [tabParam, activeTab]);
  ...
  // Sync session and strip success parameter after successful purchase
  useEffect(() => {
    if (successParam === 'true' && !successHandledRef.current) {
      successHandledRef.current = true;

      // 1. Clean up URL parameters instantly
  ...
  }, [successParam, update, router, projectId]);

  // Checkout Redirect Controller
  useEffect(() => {
  ...
  const handleTabChange = (tab: VisualTab) => {
  ...
  return (
    <div className="flex h-[100dvh] bg-black text-zinc-300 font-sans overflow-hidden relative">
      ...
      <main className="flex-1 flex flex-col relative bg-[url('/noise.png')] opacity-95 min-w-0 h-full">
        {/* Unified Top Navigation Header (incorporating Mobile toggle) */}
        <header className="h-14 border-b border-white/10 glass backdrop-blur-md flex items-center justify-between px-4 md:px-6 text-[10px] font-mono uppercase text-zinc-500 z-30 relative shrink-0">
          <div className="flex items-center">
            <button
              onClick={() => setIsSidebarOpen(true)}
              className="md:hidden mr-3 p-1.5 hover:bg-white/10 text-zinc-400 hover:text-white rounded-full border border-white/10 transition-colors shadow-[0_0_15px_rgba(255,255,255,0.05)]"
            >
              <Menu className="w-4 h-4" />
            </button>
            Vision <ChevronRight className="w-3 h-3 mx-2" /> <span className="text-white">{activeTab === 'imagine' ? 'Imagine' : activeTab === 'profile' ? 'Profile' : 'Dashboard'}</span>
          </div>
          <div className="flex items-center">
            <span className="text-[8px] md:text-[10px] font-bold text-white tracking-widest px-4 py-2 rounded-full border border-white/10 shadow-[0_0_20px_rgba(255,255,255,0.03)] bg-white/5">
        ...
        {activeTab === 'imagine' ? (
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6 w-full custom-scrollbar">
            <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-4xl mx-auto space-y-6 pb-10 mt-4">
              ...
              <div className="glass-dark border border-white/10 rounded-3xl p-6 flex flex-col md:flex-row items-center md:items-start gap-6 relative overflow-hidden group shadow-[0_0_30px_rgba(0,0,0,0.5)]">
                ...
                <div className="flex-1 space-y-3 text-center md:text-left z-10">
                  ...
                  <div className="flex flex-col md:flex-row gap-3 items-center md:items-start">
                    <div className="flex items-center gap-2 text-zinc-400 bg-white/5 px-4 py-2 rounded-full border border-white/5 text-[10px] font-mono uppercase tracking-wider">
                  ...
                  <div className="bg-black/60 border border-white/5 rounded-3xl p-5 flex flex-col justify-between group/plan transition-all hover:border-white/20">
                    <div>
                      ...
                      <div className="flex items-center gap-3 mb-6">
                        <span className={cn("text-xl font-mono font-bold uppercase tracking-widest", userState.tier !== 'FREE' ? 'text-white ' : 'text-zinc-400')}>
                          {userState.tier}
                        </span>
                        {userState.tier !== 'FREE' && <span className="bg-white text-black text-[9px] font-mono uppercase tracking-widest px-2.5 py-0.5 rounded-full shadow-[0_0_10px_rgba(255,255,255,0.2)]">ACTIVE</span>}
                      </div>
                    </div>
                    {userState.tier === 'FREE' ? (
                      <button
                        onClick={() => setShowPlanModal(true)}
                        className="w-full py-2.5 bg-white text-black hover:bg-zinc-200 text-xs font-mono uppercase tracking-[0.2em] font-bold rounded-full transition-all shadow-[0_0_20px_rgba(255,255,255,0.15)] hover:shadow-[0_0_30px_rgba(255,255,255,0.25)]"
                      >
                        Upgrade Plan
                      </button>
                    ) : (
                      <div className="space-y-2">
                        {userState.cancelAtPeriodEnd ? (
                          <div className="w-full py-2.5 bg-red-500/10 text-red-400 text-xs font-mono uppercase tracking-[0.2em] font-bold rounded-full border border-red-500/20 text-center flex items-center justify-center gap-2">
                            <AlertTriangle className="w-3.5 h-3.5 animate-pulse" /> Cancels at Period End
                          </div>
                        ) : (
                          <>
                            <button
                    ...
                    <div>
                      ...
                      <div className="flex justify-between items-end mb-3">
                        <span className="text-2xl font-mono text-white tracking-tighter ">
                          {userState.usage.toLocaleString()} <span className="text-[10px] text-zinc-400 uppercase tracking-widest">pulse</span>
                        </span>
                        <span className="text-[10px] font-mono text-zinc-400 uppercase tracking-widest">Limit: {userState.limit.toLocaleString()}</span>
                      </div>
                      <div className="w-full h-1.5 bg-zinc-900 rounded-full mb-4 overflow-hidden border border-white/5">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${Math.min((userState.usage / (userState.limit === Infinity ? 100 : userState.limit)) * 100, 100)}%` }} transition={{ duration: 1, ease: "easeOut" }} className={cn("h-full rounded-full shadow-[0_0_10px_rgba(255,255,255,0.2)]", (userState.limit !== Infinity && userState.usage >= userState.limit) ? "bg-red-500" : "bg-white text-black")} />
                      </div>
                    </div>
                    <p className="text-[9px] font-mono text-zinc-400 uppercase tracking-widest">Resets at the start of next billing cycle.</p>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          <div className="flex items-center justify-center h-full text-zinc-400 font-mono text-sm uppercase tracking-widest">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/90 backdrop-blur-md z-[100] flex items-center justify-center md:p-4" onClick={() => setShowPlanModal(false)}>
            <motion.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.98, opacity: 0 }} onClick={(e) => e.stopPropagation()} className="glass border-y md:border border-white/10 md:rounded-[2rem] max-w-5xl w-full h-[100dvh] md:h-auto md:max-h-[90vh] overflow-auto shadow-[0_0_50px_rgba(0,0,0,1)] custom-scrollbar relative">
              ...
              <div className="px-6 pt-6 pb-4 flex items-center justify-between border-b border-white/10 sticky top-0 z-20 bg-black/80 backdrop-blur-md">
                ...
                <button onClick={() => setShowPlanModal(false)} className="p-2 hover:bg-white/10 text-zinc-500 hover:text-white rounded-full transition-all">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="flex justify-center my-6 relative z-10">
                <div className="bg-black/50 border border-white/10 p-1.5 rounded-full flex gap-1">
                  <button
                    onClick={() => setBillingCycle('monthly')}
                    className={cn("px-6 py-2.5 text-xs font-mono uppercase tracking-widest rounded-full transition-all", billingCycle === 'monthly' ? "bg-white text-black font-bold shadow-[0_0_15px_rgba(255,255,255,0.2)]" : "text-zinc-500 hover:text-white")}
                  >
                    Monthly
                  </button>
                  <button
                    onClick={() => setBillingCycle('yearly')}
                    className={cn("px-6 py-2.5 text-xs font-mono uppercase tracking-widest rounded-full transition-all relative", billingCycle === 'yearly' ? "bg-white text-black font-bold shadow-[0_0_15px_rgba(255,255,255,0.2)]" : "text-zinc-500 hover:text-white")}
                  >
                    Yearly
                    <span className="absolute -top-3.5 -right-3.5 bg-white text-black text-[8px] font-bold px-2 py-0.5 rounded-full shadow-[0_0_10px_rgba(255,255,255,0.2)]">SAVE 16%</span>
                  </button>
                </div>
              </div>

              {/* Pricing Cards Grid using custom rounded visual tokens */}
              <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pb-20 md:pb-10 relative z-10 pt-0">
                {PLANS.map((plan) => {
                  ...
                  return (
                    <div key={plan.id} className={cn("rounded-3xl border p-5 transition-all flex flex-col justify-between relative group", isCurrent ? "border-white/30 shadow-[0_0_20px_rgba(255,255,255,0.05)] bg-white/[0.02]" : "border-white/10 hover:border-white/20 bg-white/[0.01]")}>
                      <div>
                        {plan.popular && <div className="text-[9px] font-mono uppercase tracking-widest text-black bg-white inline-block px-3 py-1 rounded-full mb-3 shadow-[0_0_10px_rgba(255,255,255,0.25)] font-bold">RECOMMENDED</div>}
                        <div className="text-xs md:text-sm font-mono uppercase tracking-widest text-white mb-1 font-bold">{plan.name}</div>
                        ...
                        <p className="text-[9px] md:text-[10px] font-mono uppercase tracking-widest text-zinc-400 mb-6 leading-relaxed">{plan.desc}</p>
                        <ul className="space-y-3 mb-8">
                          {plan.features.map((f, i) => (
                            <li key={i} className="flex items-start gap-2.5 text-[9px] md:text-[10px] font-mono uppercase tracking-wider text-zinc-400">
                              <Check className="w-3.5 h-3.5 text-white shrink-0 mt-0.5" /> <span className="group-hover:text-zinc-200 transition-colors">{f}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
                      <button
                        onClick={() => {
                          ...
                          if (plan.id !== 'FREE') {
                            setCheckoutPlanContext(plan.id);
                            setShowCheckoutModal(true);
                            setCheckoutTermsAgreed(false);
                            setCheckoutTermsConfirmed(false);
                          }
                        }}
                        disabled={isCurrent}
                        className={cn("w-full py-3 font-mono text-[9px] uppercase font-bold rounded-full transition-all", isCurrent ? "glass text-zinc-600 cursor-not-allowed border-white/5" : "bg-white text-black hover:bg-zinc-200 shadow-[0_0_15px_rgba(255,255,255,0.1)] hover:shadow-[0_0_20px_rgba(255,255,255,0.2)] active:scale-95")}
                      >
      ...
      <AnimatePresence>
        {showCheckoutModal && checkoutPlanContext && (
          ...
          >
            <div className="glass-dark border border-white/10 w-full max-w-5xl rounded-[2rem] relative flex flex-col md:flex-row overflow-hidden shadow-[0_0_50px_rgba(0,0,0,1)]">
              {/* Close Button */}
              <button
                onClick={() => setShowCheckoutModal(false)}
                className="absolute top-5 right-5 text-zinc-500 hover:text-white z-20 bg-black/50 md:bg-transparent p-2 rounded-full transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="w-full md:w-1/3 glass p-6 md:p-8 flex flex-col border-b md:border-b-0 md:border-r border-white/10 relative overflow-hidden">
                ...
                {PLANS.filter(p => p.id === checkoutPlanContext).map(selectedPlan => {
                  ...
                  const period = billingCycle === 'monthly' ? '/mo' : '/yr';
                  return (
                    <div key={selectedPlan.id} className="h-full flex flex-col justify-between">
                      <div>
                        ...
                        <div className="relative z-10">
                          <p className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest mb-4">Included capabilities:</p>
                      ...
                      <div className="mt-8 pt-6 border-t border-white/5 flex items-center gap-2 text-zinc-500">
              ...
              <div className="w-full md:w-2/3 bg-black/40 relative min-h-[500px] flex flex-col items-center justify-center p-6 md:p-10">
                {!checkoutTermsConfirmed ? (
                  <div className="flex flex-col items-center justify-center h-full max-w-sm text-center mx-auto space-y-6">
                    ...
                    <label className="flex items-start gap-3 cursor-pointer group bg-white/5 hover:bg-white/10 p-4 border border-white/10 rounded-3xl transition-colors text-left w-full mt-4">
                      <div className="relative flex items-center justify-center mt-0.5">
                        <input
                          type="checkbox"
                          className="peer appearance-none w-4 h-4 border border-white/20 rounded-full bg-black checked:bg-white checked:border-white transition-colors"
                      ...
                      <div className="text-[10px] font-mono text-zinc-400 tracking-wider leading-relaxed">
                    ...
                    <button
                      ...
                      className={cn(
                        "w-full py-3.5 font-mono text-[10px] md:text-xs uppercase font-bold tracking-widest rounded-full transition-all duration-300 mt-2",
                        checkoutTermsAgreed
                          ? "bg-white text-black hover:bg-white/90 border border-transparent shadow-[0_0_30px_rgba(255,255,255,0.1)] active:scale-95"
                  ...
                  <div className="flex flex-col items-center justify-center h-full max-w-sm text-center mx-auto space-y-8">
                    ...
                    <div className="w-full space-y-4">
                      <button
                        ...
                        className="w-full py-4 bg-white text-black hover:bg-zinc-200 font-mono text-xs uppercase font-bold tracking-widest rounded-full transition-all shadow-[0_0_30px_rgba(255,255,255,0.1)] hover:shadow-[0_0_40px_rgba(255,255,255,0.2)] flex items-center justify-center gap-3 group active:scale-95"
                      >
                        <CreditCard className="w-4 h-4 shrink-0" />
                        Pay with Card / Mobile / PayPal
                      ...
                      <button
                        onClick={() => setIsRedirecting(true)}
                        className="w-full py-3.5 bg-black/40 border border-white/10 text-zinc-400 hover:text-white hover:border-white/40 font-mono text-[10px] uppercase tracking-[0.2em] rounded-full transition-all flex items-center justify-center gap-3 group active:scale-95"
                      >
      ...
      <AnimatePresence>
        {showSuccessModal && (
          <div className="fixed inset-0 bg-black/90 z-[200] flex items-center justify-center backdrop-blur-md">
            <div className="bg-black border border-white/20 p-8 text-center max-w-sm rounded-[2rem] shadow-2xl">
              <CheckCircle2 className="w-16 h-16 text-white mx-auto mb-4 text-glow-white" />
              <h2 className="text-xl font-mono text-white uppercase tracking-widest mb-2">Payment successful</h2>
              <p className="text-[10px] font-mono uppercase tracking-widest text-zinc-500 leading-relaxed mb-6">Your capability matrix has been upgraded successfully.</p>
              <button
                onClick={() => {
                  setShowSuccessModal(false);
        ...
        {showGlobalTermsModal && (
          ...
          >
            <motion.div
              ...
              className="glass border border-white/10 w-full max-w-lg rounded-[2rem] relative shadow-[0_0_50px_rgba(0,0,0,1)] overflow-hidden"
            >
              <div className="p-8 md:p-10 flex flex-col items-center text-center">
                <div className="w-16 h-16 bg-white/5 border border-white/10 rounded-full flex items-center justify-center mb-6 shadow-[0_0_20px_rgba(255,255,255,0.05)]">
                ...
                <p className="text-[10px] font-mono uppercase tracking-widest text-zinc-400 leading-relaxed mb-8">
                  Welcome to VoiceLab AI. Before you can access the vision dashboard, you must read and agree to our operational guidelines and privacy protocols.
                </p>

                <div className="w-full space-y-4 mb-8">
                  <Link href="/terms-of-use" target="_blank" className="flex items-center justify-between p-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full transition-colors group px-6">
                  ...
                  <Link href="/privacy-policy" target="_blank" className="flex items-center justify-between p-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full transition-colors group px-6">
                    <div className="flex items-center gap-3">
                ...
                <label className="flex items-start gap-3 cursor-pointer group text-left w-full mb-8 bg-white/5 p-4 border border-white/10 rounded-[1.5rem] transition-colors hover:bg-white/10">
                  <div className="relative flex items-center justify-center mt-0.5 shrink-0">
                    <input
                      type="checkbox"
                      className="peer appearance-none w-5 h-5 border border-white/20 rounded-full bg-black checked:bg-white checked:border-white transition-colors cursor-pointer"
                ...
                <button
                  ...
                  className={cn(
                    "w-full py-4 font-mono text-xs uppercase font-bold tracking-widest rounded-full transition-all duration-300",
