...
const ScrollLiquidBackground = () => {
  ...
  // FIXED: Changed fixed vw values to relative max-sizes to prevent horizontal overflow on mobile
  return (
    <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden flex items-center justify-center">
      <motion.div
        style={{ rotate: rotate1, scale: scaleMorph }}
        className="absolute w-[120%] md:w-[82vw] aspect-square max-w-[1000px] bg-gradient-to-br from-cyan-500/25 via-indigo-600/20 to-transparent blur-[80px] md:blur-[130px] rounded-[45%_55%_60%_40%/50%_40%_55%_45%] mix-blend-screen opacity-70"
      />
      <motion.div
        style={{ rotate: rotate2, scale: scaleMorph }}
        className="absolute w-[90%] md:w-[58vw] aspect-square max-w-[700px] bg-gradient-to-tr from-white/10 via-cyan-400/15 to-transparent blur-[60px] md:blur-[110px] rounded-[55%_45%_40%_60%/40%_55%_45%_60%] mix-blend-overlay opacity-75"
      />
      <motion.div
        style={{ rotate: rotate3, scale: scaleMorph }}
        className="absolute w-[140%] md:w-[95vw] aspect-square max-w-[1200px] bg-gradient-to-r from-indigo-400/10 to-transparent blur-[100px] md:blur-[160px] rounded-[30%_70%_65%_35%/60%_30%_70%_40%] mix-blend-soft-light opacity-40"
      />
    </div>
  );
};
  ...
  return (
    <div className="relative w-full flex flex-col items-center justify-center py-6 md:py-8 overflow-hidden">
      ...
      <div className="flex items-end justify-center gap-[2px] md:gap-[3px] h-24 md:h-40 relative z-10 w-full px-2 overflow-hidden">
        {bars.map((i) => (
          <motion.div
            ...
            animate={{
              height: [14, 68, 22, 82, 18, 55, 28, 71, 15].map(h => h * 0.8), // Slightly shorter for mobile safety
            }}
            transition={{
              duration: 1.35,
              repeat: Infinity,
      ...
      <div className="flex items-center gap-1.5 md:gap-2 mt-4 md:mt-6">
        ...
        <p className="text-[8px] md:text-[10px] font-mono tracking-[1px] md:tracking-[2px] uppercase text-cyan-400">
  ...
  return (
    <div className="relative w-full h-36 md:h-48 flex items-center justify-center overflow-hidden py-4">
      <motion.div
        ...
        className="absolute w-16 h-16 md:w-20 md:h-20 bg-cyan-500/20 rounded-full blur-xl md:blur-2xl"
      />
      {[0, 1, 2].map((i) => (
        <motion.div
          key={i}
          className="absolute w-8 h-8 md:w-12 md:h-12 border border-cyan-400/60 rounded-full"
          animate={{
            scale: [1, 4],
            opacity: [0.8, 0],
          }}
      ...
      <motion.div
        ...
        className="absolute w-28 h-28 md:w-36 md:h-36 border border-white/20 border-dashed rounded-full"
      />

      <motion.div
        animate={{ rotate: -360 }}
        transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
        className="absolute w-20 h-20 md:w-24 md:h-24 border border-cyan-400/30 rounded-full flex items-center justify-center"
      >
    ...
    <div className="min-h-screen bg-black text-zinc-300 font-sans relative selection:bg-white/20 overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-black/80 backdrop-blur-md">
        <div className="max-w-screen-2xl mx-auto px-4 md:px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2 md:gap-3 group cursor-pointer">
            ...
            <span className="text-[8px] md:text-[9px] px-1.5 py-0.5 bg-white/5 text-zinc-500 rounded-sm border border-white/10 uppercase tracking-widest hidden sm:inline-block">Core</span>
          </div>
          ...
          <div className="flex items-center gap-4">
            <Link
              href="/dashboard"
              className="px-3 md:px-4 py-1.5 md:py-2 bg-white text-black text-[9px] md:text-[10px] font-mono font-bold uppercase tracking-widest rounded-sm flex items-center gap-2 hover:bg-cyan-300 transition-all"
            >
              App
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <motion.div
              ...
              className="bg-black/60 backdrop-blur-sm border border-white/10 rounded-sm p-5 md:p-6 transition-all duration-300 hover:border-cyan-400 hover:bg-white/5 group flex flex-col items-center text-center md:items-start md:text-left"
            >
      ...
      <div id="fluid" className="relative z-10 w-full py-20 md:py-32 border-t border-white/5 bg-black/20 backdrop-blur-sm">
        <div className="max-w-screen-xl mx-auto px-4 md:px-8 text-center flex flex-col items-center">
          ...
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-mono font-bold text-white uppercase tracking-tighter mb-3 md:mb-4">
            Fluid_Integration
          </h2>
      ...
      <div id="pricing" className="relative z-10 max-w-screen-xl mx-auto px-4 md:px-8 py-16 md:py-24 border-t border-white/5 bg-black/60 backdrop-blur-md">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          ...
          <div className="bg-black border border-white/10 rounded-sm p-6 md:p-8 flex flex-col hover:border-white/30 transition-colors backdrop-blur-md">
            ...
            <ul className="space-y-3 md:space-y-4 mb-6 md:mb-8 flex-1">
              {['5 generations/month', '50 MB files', '5,000 characters TTS'].map((feature, i) => (
                <li key={i} className="flex items-start gap-2 md:gap-3 text-[9px] md:text-[10px] font-mono uppercase tracking-wider text-zinc-400">
          ...
          <div className="bg-white/5 border border-cyan-400/40 rounded-sm p-6 md:p-8 flex flex-col relative transform lg:-translate-y-4 shadow-[0_0_30px_rgba(103,232,249,0.15)] backdrop-blur-md mt-4 sm:mt-0">
            <div className="text-[8px] md:text-[9px] font-mono uppercase tracking-widest text-black bg-white inline-block px-2 py-0.5 md:py-0.5 mb-3 md:mb-4 self-start absolute top-0 -translate-y-1/2 left-6 md:left-8">
              RECOMMENDED
            </div>
            ...
            <ul className="space-y-3 md:space-y-4 mb-6 md:mb-8 flex-1">
              {['20 generations/month', '300 MB files', '10,000 characters TTS', 'Commercial license'].map((feature, i) => (
                <li key={i} className="flex items-start gap-2 md:gap-3 text-[9px] md:text-[10px] font-mono uppercase tracking-wider text-white">
                  <Check className="w-3 h-3 text-white shrink-0 mt-0.5" /> {feature}
                </li>
              ))}
            </ul>
      ...
      <footer className="relative z-10 border-t border-white/5 bg-black py-8 md:py-12">
        <div className="max-w-screen-2xl mx-auto px-4 md:px-8 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            <Link href="#" className="p-2 md:p-2.5 rounded-sm hover:border-cyan-400 hover:text-cyan-400 text-zinc-500 transition-all group">
